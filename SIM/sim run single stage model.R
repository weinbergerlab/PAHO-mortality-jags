...
sim.counts<-readRDS( file=paste0(input.dir,"sim.counts.", ag.select,".rds"))
    ...
    output_directory <- paste(output_directory,  '/',ag.select, '.single.model.SIM' ,'/', sep = '')                     #Adds a subfolder to output directory to organize results by date and time run.
